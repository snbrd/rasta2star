...
import zionLionsPoolABI from 'config/abi/zion_adventure.json'
import { AbiItem } from 'web3-utils'
import { SEC_PER_YEAR } from 'config'
import useContract, { useZION } from 'hooks/useContract'
...
const ExplorerPoolCard: React.FC<HarvestProps> = ({ pool, removed = false }) => {
  ...
  const poolContract = useContract(zionLionsPoolABI as unknown as AbiItem, getAddress(contractAddress))
  ...
  }, [account, poolContract, zionContract])
      ...
      >
        ...
        <div className="flex gap-3 md:gap-6 flex-col md:flex-row">
          ...
          <div className="flex flex-col justify-between md:w-2/5">
            ...
            >
              ...
              {/* <span className="apr-label text-blue-zion_cyan text-sm">$ZION Staked</span> */}
              <span className="apr-label text-newpurple-400 text-sm">$ZION Staked</span>
            </div>
          </div>
        </div>
        <div className="w-full mt-10 md:mt-12 gap-4 flex flex-col md:flex-row items-center">
          ...
          <h2 className="text-md w-full text-center">
            Please note that unstaking your explorers will unstake your $ZION
          </h2>
        </div>
      </div>
    </>
